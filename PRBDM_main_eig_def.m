...
parfiles = {strcat('SimMatrixFun/B',num2str(n),'.m'),strcat('SimMatrixFun/C',num2str(n),'.m'),...
    strcat('SimMatrixFun/D',num2str(n),'.m'),strcat('SimMatrixFun/J',num2str(n),'.m'),...
    strcat('SimMatrixFun/G',num2str(n),'.m'),strcat('SimMatrixFun/visualize',num2str(n),'.m'),...
    strcat('dynamicsf.m')}
...
b_eq(1) = 1;
space = 5;
lb = [1e-5*ones(1,size(Input.x,2)) ,1/(10*space)*ones(1,(length(design_init)-size(Input.x,2)))];
...
opts = optimoptions('paretosearch','Display','diagnose','ConstraintTolerance', 1e-9,'MaxTime',24*60*60,'PlotFcn',{'psplotparetof'},'UseParallel',true)%
...
FileName=['results/PS',datestr(now, 'yyyy-mm-dd_HH_MM')]
